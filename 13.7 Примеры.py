...
# list_ = [-5, 2, 4, 8, 12, -7, 5] # Не работает код как должен
...
# Задание 13.7.1 Начинающий программист написал программу, которая находит индекс последнего отрицательного элемента
# в списке. Но он не знал, что есть функция enumerate. Ваша задача — подправить код так, чтобы он работал с помощью
# функции enumerate
# list_ = [-5, 2, 4, 8, 12, -7, 5]
...
# Решение #Никакой разницы не увидел. Явно не доработанный код.
# list_ = [-5, 2, 4, 8, 12, -7, 5]
# # Объявим переменную, в которой будем хранить индекс отрицательного элемента
# index_negative = None
#
...
# Пример. Проверьте, является ли заданное число степенью тройки.
# while True:
#     if n % 3 == 0:
#          n = n // 3
#          if n == 1:
...
# Гипотеза Сиракуз заключается в том, что любое натуральное число можно свести к 1, если повторять над ним следующие
# действия:
